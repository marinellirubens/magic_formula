...
async def main():
    ...
    tickers_df: DataFrame = await process_tickers(stock_tickers, roic_index_info)
...
def set_logger():
    ...
    LOG_FILENAME = 'stocks.log'
    formatter = logging.Formatter(
        '%(asctime)s - %(name)s - (%(threadName)-10s) - %(levelname)s - %(message)s')
    handler = logging.handlers.RotatingFileHandler(LOG_FILENAME, maxBytes=1024 * 1000,
...
async def ticker_is_valid(symbol, logger):
...
async def return_earning_yield(symbol: str, tickers: list, df: DataFrame,
                         index: int, roic_index: dict, LOGGER):
    """Rules:
    ...
    """
    ticker_valid, ticker_info = await ticker_is_valid(symbol, LOGGER)
    if not ticker_valid:
        return False
    ...
    if earning_yield > 0:
        LOGGER.info(f'Inserting ticker: {symbol} on dataframe')
...
def get_ticker_roic_info(URL: str):
...
async def process_tickers(stock_tickers: set, roic_index: dict):
    ...
    for index, ticker in enumerate(stock_tickers):
        ...
        task = loop.create_task(return_earning_yield(
            ticker + '.SA', tickers, df, index, roic_index, LOGGER
            )
        )
        LOGGER.info(f'Processing ticker: {ticker} on thread {task}')
        tasks.append(task)
